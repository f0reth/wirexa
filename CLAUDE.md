...
**Persistence.** User data lives under `os.UserConfigDir()/Wirexa/`: per-entity JSON directories (`collections/`, `mqtt-profiles/`, `udp-targets/`), plus `sidebar_layout.json`, `window-state.json`, `openapi-recents.json`, and `logs/`. `infrastructure.JSONStore[T]` writes atomically and quarantines unparseable files to `*.corrupt` instead of failing startup; `application/store.CachedStore[T]` wraps any `Load/Save/Delete` repository with an in-memory map and ID assignment. Both are generic — protocol stores are instantiations, not new implementations. The exception is HTTP collections: `httpinfra.CollectionRepository` wraps `JSONStore` with storage DTOs so session file tokens never reach disk. Spilled HTTP response bodies are a second exception: they live under `os.UserCacheDir()/Wirexa/http-sessions` instead, not `os.UserConfigDir()/Wirexa/` — that data is ephemeral and can contain sensitive response content (auth tokens, PII), and cache directories are the OS-conventional place for such data and are typically excluded from backup tools, unlike the config dir.

**Security boundary.** Wails RPC is callable by any JS in the WebView, so handlers that touch the filesystem must not trust paths from the frontend. `OpenAPIHandler` keeps a `granted` allowlist seeded only from OS file dialogs and persisted recents. HTTP request files use opaque session tokens instead of paths: `HTTPHandler.OpenFilePicker(hint)` registers only the dialog's result in `httpinfra.FileRegistry` (the hint just sets where the dialog opens), and the transport resolves the token right before reading. Collections persist through `httpinfra.CollectionRepository`, whose storage DTOs cannot hold tokens or paths; legacy paths migrate to basename + "reselect". `docs/http-local-file-access-hardening.md` is the design (it records the remaining TOCTOU risk), and `docs/backend-architecture-review.md` has the wider findings list. Treat those two docs as the plan of record when touching file I/O.

**Large responses.** `httpinfra.NetClient` spills over-limit response bodies into `0700` per-request session dirs under the app-managed `os.UserCacheDir()/Wirexa/http-sessions` (not the shared OS temp dir), tracked by `ResponseStore`, keyed by the per-send execution ID (`HTTPRequest.ID`). The frontend never sees the path; it calls `SaveResponseBody(executionID)` / `DiscardResponseBody(executionID)`, and the store bounds file count, total size, concurrent spills and TTL on its own. Leftovers are cleared by `Cleanup()` on shutdown (after `HTTPRequestService.Shutdown` cancels in-flight requests) and `SweepStaleTempFiles(sessionDir)` at startup — the sweep only deletes a `wirexa-http-*` directory whose marker file matches a per-install random secret persisted alongside the session dirs (`crypto/rand`, generated on first use), not a fixed compile-time constant, since the single-instance lock only rules out a second Wirexa process, not an unrelated same-user process forging the marker.
