...
**Large responses.** `httpinfra.NetClient` spills over-limit response bodies into a `0700` session dir tracked by `ResponseStore`, keyed by the per-send execution ID (`HTTPRequest.ID`). The frontend never sees the path; it calls `SaveResponseBody(executionID)` / `DiscardResponseBody(executionID)`, and the store bounds file count, total size, concurrent spills and TTL on its own. Leftovers are cleared by `Cleanup()` on shutdown (after `HTTPRequestService.Shutdown` cancels in-flight requests) and `SweepStaleTempFiles()` at startup (safe because the single-instance lock guarantees no peer process).
